...
function initEnquadramentoCursos() {
    ...
    btnStart.addEventListener('click', () => {
        ...
        consoleLogs.innerHTML = `
            <div class="flex gap-2 mb-1 text-[14px]">
                ...
                <span class="text-purple-900 font-bold">init_enquadramento_cursos --verbose</span>
            </div>
        ...
        try {
            ...
            console.warn("Filtros HTML não encontrados, enviando padrão.");
            payloadConfig = { documentos: ['CONTRATOS'], anos: ['TODOS'], semestres: ['TODOS'], gerar_relatorio: false };
        }

        fetch('/automacoes/enquadramento-cursos/api/iniciar-processamento/', {
            method: 'POST',
    ...
    btnStop.addEventListener('click', () => {
        ...
        if (currentId) {
            fetch(`/automacoes/enquadramento-cursos/api/parar-processamento/${currentId}/`, { method: 'POST' })
                .then(res => res.json())
    ...
    function iniciarMonitoramento(processo_id) {
        pollInterval = setInterval(() => {
            fetch(`/automacoes/enquadramento-cursos/api/status-processamento/${processo_id}/`)
                .then(res => {
                    if (!res.ok) {
                    ...
                    if (data.status_codigo === 'CONCLUIDO' || data.status_codigo === 'FALHA') {
                        ...
                        if (data.status_codigo === 'CONCLUIDO') {
                            ...
                            if (rawLog.includes('FILTRO VAZIO') || rawLog.includes('Nenhum dado processado')) {
                                ...
                                btnDownload.onclick = () => {
                                    const downloadUrl = `/automacoes/enquadramento-cursos/api/baixar-resultado/${processo_id}/`;
...
document.addEventListener('turbo:load', initEnquadramentoCursos);
if (document.readyState !== 'loading') {
    initEnquadramentoCursos();
} else {
    document.addEventListener('DOMContentLoaded', initEnquadramentoCursos);
}