...
# Carrega variáveis de ambiente (.env). override=True é essencial: este processo nasce
# via subprocess.Popen a partir do worker Django, herdando o ambiente que esse worker já
# tinha em memória desde o boot — sem override=True, uma senha trocada no .env só passaria
# a valer depois de reiniciar o servidor inteiro.
            ...
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    try:
                        ...
                        pass # Ignora se não houver popup ou se já tiver passado

                    btn_baixar = frame.locator("#idBtnDown:not(.disabled)")
                    btn_baixar.wait_for(state="visible", timeout=300000)
...
def extrair_ano_pagamento(ano_str, pasta_pagamentos):
    ...
    tag = formatar_tag("DOCUMENTO AULIXIAR", "GESTÃO BOLSA", ano_str)
...
def macro_processar_pagamentos(anos_necessarios, pasta_pagamentos):
    if not anos_necessarios: return
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        futures = [executor.submit(extrair_ano_pagamento, str(ano), pasta_pagamentos) for ano in anos_necessarios]
        for f in concurrent.futures.as_completed(futures):
            try: f.result()
...
# ==========================================
def executar(docs_selecionados=None, anos_selecionados=None, periodos_selecionados=None, processo_id=None):
    if not processo_id:
        raise ValueError("O processo_id é obrigatório.")
    tempo_inicio_global = time.time()
    print("🚀 Iniciando limpeza e recriação dos diretórios de extração...")
    ...
    for doc in CONFIG_DOCUMENTOS:
        ...
        for ano in sorted(list(set(anos_selecionados))):
            for periodo in sorted(list(set(periodos_selecionados))):
                ...
                if doc["categoria"] == "HISTORICO" and sem == "2025-1":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as macro_executor:
        ...
        t_pag = macro_executor.submit(macro_processar_pagamentos, list(anos_reais_pagamentos), pasta_pagamentos)
        concurrent.futures.wait([t_analise, t_pag])
    ...
    total_baixado = conta_arquivos(tarefas_menus[0]["pasta_raiz"]) + conta_arquivos(pasta_pagamentos)
