...
def get_configs(processo_id):
    ...
    return [
        ...
        },
        {
            ...
            'ordem_colunas': [
                'Status_IA', 'Gemini Inconsistencias', 'Perfil do Beneficiario', 'Inscrição', 'Bolsista', 'Bolsistas', 'CPF', 
        ...
        },
        {
            'tipo': 'proc_geral',
            'pasta': f'{base_dir}/analise_documentos_processados',
            'subpastas_alvo': ['BENEFICIOS', 'CONTRATOS', 'FINANCIAMENTO', 'HISTORICO'],
            'saida': 'consolidado_processados.xlsx',
            'aba': 'Processados',
            'ext': '.xlsx',
            'cols_num': ['Inscrição', 'CPF', 'Coleta ID', 'Gemini CPF'],
                        ...
                        if df is not None:
                            ...
                            for col in cf['cols_num']:
                                if col in df.columns:
                                    ...
                                    # Parse to float first, then filter out values too large for Int64, then cast
