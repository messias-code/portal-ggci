...
from apps.automacoes.enquadramento_cursos.models import ProcessamentoEnquadramentoCursos
from apps.automacoes.enquadramento_cursos.services import extrator, consolidador, ggci
...
PROGRESS_MAP_ABSOLUTE = [
    ...
    ("Conectando ao banco de dados para buscar situação cadastral",             63),
    ("Situações carregadas com sucesso",                66),
    ("Mesclando bases e calculando Status de Vínculo (Documentos)", 70),
    ("Mesclando bases e calculando Status de Vínculo (RIAF)",       76),
    ("Agrupando planilhas por Faculdade e Semestre",    85),
    ("Finalizando e gerando o pacote ZIP",              95),
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            proc = ProcessamentoEnquadramentoCursos.objects.get(id=processo_id)
        ...
        try:
            ...
            anos_para_validar = anos if anos and "TODOS" not in anos else ["2025", "2026"]
            sems_para_validar = sems if sems and "TODOS" not in sems else ["1", "2"]
            tem_multiplos_periodos = len(anos_para_validar) > 1 or len(sems_para_validar) > 1
            pode_gerar_relatorio = (
                ...
                tem_multiplos_periodos
            )
            if gerar_relatorio and not pode_gerar_relatorio:
                registrar_log(f"[AVISO | RELATÓRIO | BLOQUEADO] ⚠️ A aba 'Relatório' exige: Contratos, Financiamentos, Benefícios e pelo menos dois períodos para comparação (ex: dois anos ou dois semestres).")
            
            total_baixado = extrator.executar(docs_selecionados=docs, anos_selecionados=anos, periodos_selecionados=sems, processo_id=processo_id)
            ...
            consolidador.consolidar(processo_id=processo_id) # Chama a função do seu script original
            ...
            ggci.gerar_relatorio_geral(docs_selecionados=docs, anos_selecionados=anos, sems_selecionados=sems, gerar_relatorio=gerar_relatorio, processo_id=processo_id) 
