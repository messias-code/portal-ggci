"""
=== ARQUIVO: apps/automacoes/analise_ia/services/ggci.py ===
...
# ==========================================
PASTA_CONSOLIDADOS = os.path.join("apps", "automacoes", "analise_ia", "dados", "analise_documentos_processados", "CONSOLIDADO")
...
ARQ_AGENDAR = os.path.join("apps", "automacoes", "analise_ia", "dados", "analise_documentos_agendar_processamentos", "CONSOLIDADO", "consolidado_agendar_processamentos.parquet")

PASTA_PAGAMENTOS = os.path.join("apps", "automacoes", "analise_ia", "dados", "analise_pagamentos", "CONSOLIDADO")
ARQ_PAGAMENTOS = os.path.join(PASTA_PAGAMENTOS, "consolidado_pagamentos.parquet")

ARQUIVO_GERAL_SAIDA = os.path.join("apps", "automacoes", "analise_ia", "dados", "relatorio_geral.xlsx")
...
def caminho_cache_gemini():
    ...
    """
    pasta = os.path.join(PROJECT_ROOT, "apps", "automacoes", "analise_ia", "dados", "cache")
    os.makedirs(pasta, exist_ok=True)
    novo = os.path.join(pasta, f"cache_gemini_documentos{env_suffix}.parquet")

    legado = os.path.join(PROJECT_ROOT, "apps", "automacoes", "analise_ia", "dados", "tabelas_sql", "historico_ia_local.parquet")
...
def buscar_dados_financeiros_sql(semestres_presentes, inscricoes=None):
    ...
    caminho_beneficiarios = os.path.join(PROJECT_ROOT, f"apps/automacoes/analise_ia/dados/tabelas_sql/PY_ggci_coleta_de_dados_beneficiarios_temp_d1{SUFIXO_TABELAS}.parquet")
    caminho_pagamentos = os.path.join(PROJECT_ROOT, f"apps/automacoes/analise_ia/dados/tabelas_sql/PY_ggci_coleta_de_dados_pagamentos_temp_d1{SUFIXO_TABELAS}.parquet")
...
def aplicar_transicoes(df, df_pag):
    ...
    if 'CPF' in df.columns and 'data_coleta' in df.columns:
        ...
        # Mapear de volta para o DF completo usando MultiIndex para evitar InvalidIndexError
...
def calcular_auditoria_ia(df):
    ...
    def atualizar_e_aplicar_cache_local(df_documentos):
        ...
        mask_processados = (
            pd.notna(df_documentos.get('Status_IA')) & 
            (df_documentos.get('Status_IA') != 'Não Processado') & 
            (df_documentos.get('Status_IA') != '') &
        ...
        if not df_cache.empty:
            ...
            if not common_idx.empty:
                ...
                # Literais porque este módulo não tem a constante `_STATUS_SEM_DOCUMENTO`
                # do dashboard; são os mesmos dois valores usados no resto do arquivo.
                ausente_agora = st_ia.loc[common_idx].isin(['ausente', 'ausentes'])
    ...
    # --- CORREÇÃO: Atualização Bruta ---
    # Se o status veio vazio ou "Não Processado" do banco, mas a IA gerou os dados, recalcula o status.
    mask_recalcular = ((ia_status_original == '') | ia_status_upper.str.contains('NÃO PROCESSADO|NAO PROCESSADO', regex=True)) & (inc_original != '')
    ia_status_original = pd.Series(np.where(mask_recalcular, np.where(inc_original.str.contains('Sem inconsist', case=False, na=False), 'Válido', 'Inválido'), ia_status_original), index=df.index)
    ia_status_upper = ia_status_original.str.upper()
    # -----------------------------------
    ...
    # Regra de Inadimplente: Todos sem pagamentos válidos no semestre (qtd_pagtos - retroativos <= 0) OU com valor pago zerado recebem Inadimplente
    ...
    # obrigação que existisse. Quem precisa saber que o arquivo chegou e foi lido tem a coluna
    # `Status Doc` do dashboard, que continua dizendo `Processado` ao lado deste `Inadimplente`.
    ...
    # O DESCARTE POR ESTORNO COMPLETO foi removido a pedido do dono do projeto.
    #
    ...
    for col in ['Processado', 'Processar']:
...
def gerar_relatorio_geral(docs_selecionados=None, periodos_por_doc=None, gerar_relatorio=False, gerar_relatorio_riaf=False, gerar_quantitativo=True, gerar_pagamentos=True, sems_riaf=None, sems_contratos=None, processo_id=None, formato="EXCEL"):
        ...
        if check_historico and sems_alvo:
            ...
            for ano in ['2025', '2026']:
                if any(ano in str(s) for s in sems_alvo):
                    p = os.path.join(PROJECT_ROOT, f"apps/automacoes/analise_ia/dados/tabelas_sql/PY_ggci_espelho_historico_d1_{ano}{SUFIXO_TABELAS}.parquet")
                    if os.path.exists(p):
    ...
    try:
        if check_contrato and sems_alvo:
            ...
            for ano in ['2025', '2026']:
                if any(ano in str(s) for s in sems_alvo):
                    p = os.path.join(PROJECT_ROOT, f"apps/automacoes/analise_ia/dados/tabelas_sql/PY_ggci_espelho_contrato_temp_d1_{ano}{SUFIXO_TABELAS}.parquet")
                    if os.path.exists(p):
                        df_ano = pl.read_parquet(p).filter(pl.col("semestre").is_in(sems_alvo)).to_pandas()
                        dfs.append(df_ano)
    ...
    try:
        if check_financ and sems_alvo:
            ...
            for ano in ['2025', '2026']:
                if any(ano in str(s) for s in sems_alvo):
                    p = os.path.join(PROJECT_ROOT, f"apps/automacoes/analise_ia/dados/tabelas_sql/PY_ggci_espelho_financiamento_d1_{ano}{SUFIXO_TABELAS}.parquet")
                    if os.path.exists(p):
                        df_ano = pl.read_parquet(p).filter(pl.col("semestre").is_in(sems_alvo)).to_pandas()
    ...
    try:
        if check_benef and sems_alvo:
            ...
            for ano in ['2025', '2026']:
                if any(ano in str(s) for s in sems_alvo):
                    p = os.path.join(PROJECT_ROOT, f"apps/automacoes/analise_ia/dados/tabelas_sql/PY_ggci_espelho_beneficio_temp_d1_{ano}{SUFIXO_TABELAS}.parquet")
                    if os.path.exists(p):
                        df_ano = pl.read_parquet(p).filter(pl.col("semestre").is_in(sems_alvo)).to_pandas()
    ...
    try:
        if check_riaf and sems_alvo:
            ...
            for ano in ['2026']:
                if any(ano in str(s) for s in sems_alvo):
                    p = os.path.join(PROJECT_ROOT, f"apps/automacoes/analise_ia/dados/tabelas_sql/PY_ggci_espelho_riaf_d1_{ano}{SUFIXO_TABELAS}.parquet")
                    if os.path.exists(p):
                        df_ano = pl.read_parquet(p).filter(pl.col("semestre").is_in(sems_alvo)).to_pandas()
                        dfs.append(df_ano)
        ...
        if not df_ativos.empty:
            ...
            if tipos_obrigatorios_geral:
                ...
                try:
                    ...
                    dir_tabelas_sql = os.path.join(PROJECT_ROOT, "apps/automacoes/analise_ia/dados/tabelas_sql")
                    pend_tasks = [
            ...
            if check_riaf:
                ...
                try:
                    ...
                    if not df_ativos_riaf.empty:
                        ...
                        try:
                            caminho_riaf = os.path.join(PROJECT_ROOT, f"apps/automacoes/analise_ia/dados/tabelas_sql/PY_ggci_pendentes_riaf_geral{SUFIXO_TABELAS}.parquet")
    ...
    if not df_docs.empty or not df_riaf.empty or gerar_relatorio_riaf or gerar_relatorio:
        ...
        COLS_REMOVER_HISTORICO = [
            'ultimo_valor_pago_ref', 'total_bolsa_paga',
            'mensalidade_sem_desc', 'gemini_mensalidade_sem_desc', 'msd_doc',
            'mensalidade_com_desc', 'gemini_mensalidade_com_desc', 'mcd_doc',
            'valor_beneficio', 'soma_valor_beneficio', 'beneficio',
            'valor_financiamento', 'soma_valor_financiamento', 'financiamento',
            'soma_ovg_devia_pagar_sis', 'soma_ovg_devia_pagar_ia', 'soma_prejuizo_ovg',
            'soma_economia_ovg', 'diagnostico_financeiro_final'
        ]
        ...
        def preparar_aba_pagamentos():
            ...
            try:
                caminho = os.path.join(PROJECT_ROOT, f"apps/automacoes/analise_ia/dados/tabelas_sql/PY_ggci_coleta_de_dados_pagamentos_temp_d1{SUFIXO_TABELAS}.parquet")
            ...
            for doc_original, tab_name, is_enabled in doc_order:
                ...
                if not df_tipo.empty:
                    ...
                    if doc_original == DOC_HISTORICO:
                        df_tipo = df_tipo.drop(columns=COLS_CHECK + COLS_REMOVER_HISTORICO, errors='ignore')
