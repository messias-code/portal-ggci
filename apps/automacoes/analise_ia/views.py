"""
Propósito: Módulo de Views para Automação de Análise de IA.
Autor: N/A
...
def iniciar_processamento_ia(request):
    """
    O QUE FAZ: Inicia um novo processamento em background (Motor de IA).
    ...
    COMO FUNCIONA:
      ...
      2. Cria um registro em `ProcessamentoAnaliseIA` (status = PENDENTE).
      3. Gerencia a rotação de logs (limita a 10 históricos) no diretório de logs.
      4. Executa `manage.py executar_motor_ia` com subprocess, passando o ID do processo.
    ...
    if request.method == 'POST':
        ...
        pasta_logs = os.path.join(settings.BASE_DIR, "apps", "automacoes", "analise_ia", "logs")
        ...
        if os.path.exists(arquivo_log):
            try:
                ...
                # Limita o histórico físico aos últimos 3 processamentos
                if len(indices_inicio) >= 3:
                    corte_idx = indices_inicio[-2]
        ...
        comando_motor = [sys.executable, '-u', 'manage.py', 'executar_motor_ia', str(processo.id)]
...
def baixar_resultado_ia(request, processo_id):
    """
    O QUE FAZ: Fornece o arquivo Excel final gerado.
    ...
    COMO FUNCIONA: 
      ...
      2. Em falha/local, delega o envio do payload ao Nginx através do header `X-Accel-Redirect` (altíssima performance).
    PARÂMETROS: request, processo_id (int)
    """
    try:
        processo = ProcessamentoAnaliseIA.objects.get(id=processo_id)
        
        # Impede download se não concluiu ou não tem arquivo
        ...
        if os.path.exists(caminho_arquivo):
            ...
            try:
                if 'runserver' in sys.argv:
                    caminho_cf = '/tmp/cf_accel_dev.json'
                else:
                    caminho_cf = '/tmp/cf_accel.json'
                ...
                if os.path.exists(caminho_cf):
                    with open(caminho_cf, 'r') as arquivo_conf:
                        ...
                        # Prevenção de Loop de Redirecionamento e bug de extração da URL
                        if url_cf and "api.trycloudflare.com" not in url_cf and "trycloudflare.com" not in host_atual:
                            return redirect(f"{url_cf}/automacoes/analise-ia/api/baixar-resultado/{processo_id}/")
            except Exception:
                pass  # Segue para o envio local em caso de erro

            # --- Estratégia Nativa: X-Accel-Redirect (Nginx) ---
            nome_original = str(processo.arquivo_resultado or "relatorio_geral.xlsx")
            nome_arquivo_sanitizado = os.path.basename(nome_original).replace("dados_analise_ia_", "")
            
            # Prepara a Response forçando mimetype
            if nome_arquivo_sanitizado.endswith('.zip'):
                content_type = 'application/zip'
            else:
                content_type = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
                
            # Instrui o Webserver Nginx a tratar do I/O do arquivo em Produção
            if getattr(settings, 'DEBUG', False):
                # Em Dev (sem Nginx), usamos o FileResponse para fazer o stream do arquivo sem lotar a memória RAM
                ...
                response['Content-Disposition'] = f'attachment; filename="{nome_arquivo_sanitizado}"'
                response['X-Accel-Redirect'] = f"/protected-media/{processo.arquivo_resultado}"
                return response
...
def analise_ia_view(request):
    """
    O QUE FAZ: Renderiza o template do Front-end principal (SPA de IA).
    POR QUÊ EXISTE: Ponto de entrada web para a feature.
    """
    if not getattr(request.user, 'p_analise_ia', False) and request.user.usuario != 'admin@ovg.org.br':
        return redirect('inicio')
        
    return render(request, 'analise_ia/index.html')
