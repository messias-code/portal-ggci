"""
=== ARQUIVO: apps/dashboards/dash_documentos_ia/tests/test_erro_na_inconsistencia.py ===
...
Espelho do teste homônimo do analise_ia — o dashboard é a visualização daquele relatório.
"""
...
class ErroNaInconsistenciaTests(SimpleTestCase):
    ...
    # --- a MAUÁ não tem tratamento nenhum aqui -------------------------------------

    def test_maua_sem_desconto_lido_nao_vira_erro_de_catalogacao(self):
        """
        O caso que motivou a regra, e a trava contra ela voltar. Contrato da MAUÁ com
        `Gemini Mensalidade C/ Desconto` VAZIO no espelho e a IA dizendo "não localizado":
        a IA ACERTOU, o valor não está no documento — é o que o SIBU mostra.

        Existiu aqui uma "REGRA DE EXCEÇÃO MAUA" que copiava a mensalidade SEM desconto
        para essa coluna, e então esta frase virava contradição: 2.084 das 2.123 linhas do
        status, 98,2%, eram leituras corretas acusadas de erro de catalogação. A regra é
        de CÁLCULO e mora em `enquadramento_cursos`; auditoria de desempenho tem de ver o
        que a IA leu. Reescrever a coluna antes deste ponto quebra este teste.
        """
        self.assertEqual(self._status(
            Inscrição=[2081943],
            Faculdade=['MAUA FACULDADE MAUA DE GOIAS'],
            **{'Gemini Inconsistencias': [NAO_LOCALIZADO],
               'Gemini Mensalidade C/ Desconto': [0.0],
               'Mensalidade S/ Desconto': [1569.77],
               'Gemini Mensalidade S/ Desconto': [1569.77],
               'Mensalidade C/ Desconto': [1469.77]},
        ), ['Válido'])

    def test_maua_com_desconto_lido_de_verdade_ainda_e_erro_de_catalogacao(self):
        """
        Contrapeso: a correção não é anistia para a MAUÁ. Quando a IA de fato leu um valor
        e ainda assim escreveu "não localizado", a frase mente e o caso é pego como em
        qualquer outra IES — o nome da faculdade não entra na conta.
        """
        self.assertEqual(self._status(
            Inscrição=[2081944],
            Faculdade=['MAUA FACULDADE MAUA DE GOIAS'],
