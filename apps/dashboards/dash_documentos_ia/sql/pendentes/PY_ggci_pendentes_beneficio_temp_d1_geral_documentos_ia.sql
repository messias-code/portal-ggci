CREATE TABLE sibu.PY_ggci_pendentes_beneficio_temp_d1_geral_documentos_ia AS
...
WITH UniversoAtivos AS (
    ...
    WHERE cd.data_create >= '2025-01-01' 
      AND (
          cd.outros_beneficios = 'S' 
          OR cd.valor_beneficios > 0 
          OR (cd.qual_beneficios IS NOT NULL AND cd.qual_beneficios != '')
      )
)
