...
function initAlterarSenha() {
    ...
    if (!formSenha || !btnSalvarSenha || !inputNovaSenha) return;
...
// Executa imediatamente quando o script for carregado (pois tem defer no HTML)
initAlterarSenha();

// E também no caso de navegação via Turbo (se aplicável)
document.addEventListener('turbo:render', initAlterarSenha);
document.addEventListener('turbo:load', initAlterarSenha);