                            ...
                            if (rawLog.includes('FILTRO VAZIO') || rawLog.includes('Nenhum dado processado')) {
                                ...
                                btnDownload.onclick = async () => {
                                    ...
                                    if (window.showSaveFilePicker) {
                                        try {
                                            // 1. Abre a janela para o usuário escolher o nome e a pasta
                                            const handle = await window.showSaveFilePicker({
                                                suggestedName: 'relatorio_geral.xlsx',
                                                types: [{ description: 'Planilha Excel', accept: { 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': ['.xlsx'] } }]
                                            });

                                            // 2. Feedback visual: Avisa que está gravando o arquivo no SSD
                                            ...
                                            // Se o usuário clicar em "Cancelar" na janela, não fazemos nada.
                                            if (err.name !== 'AbortError') {
                                                window.location.href = downloadUrl; // Fallback de emergência
                                            }
                                        }
                                    } else {
                                        // Fallback para navegadores antigos que não suportam a janela de Salvar Como
